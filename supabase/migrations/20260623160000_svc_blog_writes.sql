-- Blog writes via SECURITY DEFINER (same pattern as app_profiles RPCs).
-- PostgREST table UPDATE can return 0 rows when RLS/grants block RETURNING even with
-- service_role policies applied. These functions run as the owner (postgres) and bypass RLS.
...
BEGIN
  ...
  SET
    ...
    category_id = CASE
      WHEN p_row ? 'category_id' AND v_category_text IS NOT NULL THEN v_category_text::uuid
  ...
  IF v_id IS NULL THEN
    RAISE EXCEPTION 'BLOG_UPDATE_NO_MATCH'
      USING ERRCODE = 'P0002',
            HINT = 'No blog row matched the given id and site_id';
  END IF;

  -- Optional SEO columns (added in later migrations)
  BEGIN
    UPDATE public.blogs
    SET
...
BEGIN
  ...
  VALUES (
    ...
    CASE WHEN v_category_text IS NOT NULL THEN v_category_text::uuid ELSE NULL END,
