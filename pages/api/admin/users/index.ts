...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    ...
    for (const u of batch) {
      ...
      users.push({
        ...
        can_manage_expenses: p.can_manage_expenses,
        can_manage_profiles: p.can_manage_profiles,
        can_manage_payroll: p.can_manage_payroll,
  ...
  if (req.method === 'POST') {
    ...
    const can_manage_expenses = Boolean(body.can_manage_expenses);

    const can_manage_profiles = Boolean(body.can_manage_profiles);

    const can_manage_payroll = Boolean(body.can_manage_payroll);
