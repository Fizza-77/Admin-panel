...
async function ensureProfileRowForUser(
  ...
  if (refetch.error) {
    const msg = formatDbError(refetch.error);
    reportError(refetch.error, { source: 'getAppProfile.refetchAfterUpsert', userId });
    return {
      permissions: permissionsWithLoadError(email, msg),
      loadError: msg,
    };
  ...
  if (!refetch.row) {
    const msg = 'Profile row missing after automatic bootstrap';
    reportError(new Error(msg), { source: 'getAppProfile.missingAfterUpsert', userId });
    return {
      permissions: permissionsWithLoadError(email, msg),
      loadError: msg,
    };
...
export async function ensureAppProfileRow(userId: string): Promise<{ ok: boolean; error: string | null }> {
  ...
  const refetch = await fetchAppProfileRow(userId);
  if (refetch.error || !refetch.row) {
    const msg = refetch.error ? formatDbError(refetch.error) : 'Profile row missing after login bootstrap';
    reportError(refetch.error ?? new Error(msg), { source: 'ensureAppProfileRow.refetch', userId });
